...
settimana = {'weekly': macronutrienti_settimali,
             'day': {
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            for row in rows:
                ...
                if not ingredienti_ricetta.get(row['nome_ricetta']):
                    ingredienti_ricetta[row['nome_ricetta']] = [f"{str(row['qta'])}gr. di {row['nome']}"]
                else:
                    ingredienti_ricetta.get(row['nome_ricetta']).append(f"{str(row['qta'])}gr. di {row['nome']}")
...
def stampa_menu(dieta_settimanale, validation_rule):
    esiste_menu_settimale = True
    ...
    if not esiste_menu_settimale:
        ...
        for week_day in cols_giorno_settimana:
            ...
            for ricetta_colazione in dieta_settimanale['day'][week_day]['pasto']['colazione']['ricette']:
                ...
                ws.insert_note(str(colonna_ricetta + str(i)), str(ingredienti_ricetta.get(ricetta_colazione.get('nome_ricetta'))))
                i = i + 1
            ...
            for ricetta_pranzo in dieta_settimanale['day'][week_day]['pasto']['pranzo']['ricette']:
                ...
                ws.insert_note(str(colonna_ricetta + str(x)), str(ingredienti_ricetta.get(ricetta_pranzo.get('nome_ricetta'))))
                x = x + 1
            ...
            for ricetta_cena in dieta_settimanale['day'][week_day]['pasto']['cena']['ricette']:
                ...
                ws.insert_note(str(colonna_ricetta + str(y)), str(ingredienti_ricetta.get(ricetta_cena.get('nome_ricetta'))))
                y = y + 1
...
if __name__ == '__main__':
    definisci_calorie_macronutrienti()
    validation_rule = stampa_ricette()
    stampa_ingredienti_ricetta()
    genera_menu(False)
    genera_menu(True)
    if not stampa_menu(settimana, validation_rule):
        stampa_lista_della_spesa(settimana.get('all_food'))